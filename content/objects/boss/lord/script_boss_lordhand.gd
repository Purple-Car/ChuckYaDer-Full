...
func snapToBody(delta: float) -> void:
	global_position = lord_body.getHandPosition(int(left_hand))

func moveToBody(delta:float) -> bool:
	var destination: Vector2 = lord_body.getHandPosition(int(left_hand))
...
func setUngrabbed() -> void:
	...
	state_machine.current_state.Transitioned.emit(state_machine.current_state, "sitting")
